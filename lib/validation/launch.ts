...
export function validateLaunchParams(
  ...
  if (formData.totalSupply === undefined || formData.totalSupply <= 0) {
    ...
    add("totalSupply", "Total supply exceeds safe integer range", "SUPPLY_TOO_LARGE");
  }
  ...
  if (
    ...
    add("lockedLiquidityPercentage", "Locked liquidity must be between 0 and 100", "LIQUIDITY_OUT_OF_RANGE");
  }
  ...
  // ─── Base fee ───
  if (formData.baseFeeBps !== undefined && (formData.baseFeeBps < 0 || formData.baseFeeBps > 10000)) {
    add("baseFeeBps", "Base fee must be between 0 and 10,000 bps (0–100%)", "BASE_FEE_OUT_OF_RANGE");
  }

  // ─── Fee-scheduler validation (DRY, shared with frontend) ───
  const feeErrors = validateFeeScheduler(
    formData.feeSchedulerConfig as Record<string, unknown> ?? {},
    formData.initialMarketCap,
    formData.marketCapRangeMax
  );
  feeErrors.forEach((err) => add(err.field, err.message, err.field.toUpperCase().replace(/[^A-Z]/g, "_")));

  // ─── Quote token ───
  if (!isValidSolanaAddress(formData.quoteTokenMint ?? "")) {
    add("quoteTokenMint", "Invalid Solana address", "QUOTE_MINT_INVALID");
  } else if (
    formData.quoteTokenMint !== "So11111111111111111111111111111111111111112" &&
    formData.quoteTokenMint !== "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"
  ) {
    add("quoteTokenMint", 'Quote token must be SOL (So1111...11112) or USDC (EPjFW...TDt1v)', "QUOTE_MINT_UNKNOWN");
  }

  // ─── Decay periods (optional) ───
  if (formData.feeDecayPeriods !== undefined && (formData.feeDecayPeriods <= 0 || formData.feeDecayPeriods > 1000)) {
    add("feeDecayPeriods", "Number of periods must be between 1 and 1000", "PERIODS_OUT_OF_RANGE");
  }
